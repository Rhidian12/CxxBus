...
std::string ParseDBusAddress() {
  ...
  if (!dbusAddress.starts_with("unix:")) {
    throw std::runtime_error{
        "Only support unix sockets for DBus-daemon connections"};
  }
...
std::string HexEncodeString(std::string const &str) {
  std::string newStr;
  std::ranges::for_each(
      str, [&newStr](unsigned char c) { newStr += std::format("{:x}", c); });
  return newStr;
}
} // namespace
...
boost::asio::awaitable<void> DBusConnection::AuthenticateDBusConnection() {
  ...
  co_await m_socket.async_send(
      std::format("AUTH EXTERNAL {}\r\n",
                  HexEncodeString(std::to_string(::getuid()))),
      boost::asio::use_awaitable);

  // Now we expect to see OK <guid>
  std::string reply{};
  co_await m_socket.async_receive(boost::asio::buffer(reply),
                                  boost::asio::use_awaitable);
...
boost::asio::awaitable<void> DBusConnection::Connect() {
  ...
  boost::asio::ip::udp::endpoint endpoint{
      boost::asio::ip::make_address(ParseDBusAddress()), 0};
  ...
  std::optional<DBusReply> reply = co_await SendMessage(
  ...
  DBusMessage message{"RequestName", "/org/freedesktop/DBus",
...
boost::asio::awaitable<void> DBusConnection::SendLoop() {
  while (true) {
    auto message =
        co_await m_sendLoop.async_receive(boost::asio::use_awaitable);

    co_await m_socket.async_send(message.Serialize(m_serial),
                                 boost::asio::use_awaitable);
  }
}
...
boost::asio::awaitable<void> DBusConnection::ReadLoop() {
  while (true) {
    ...
    if (replyChannel) {
      co_await replyChannel->async_send(boost::system::error_code{},
                                        DBusReply{reply});
    }
  }
}

boost::asio::awaitable<std::optional<DBusReply>>
DBusConnection::SendMessage(DBusMessage const &message) {
  // 1st, send our message to the SendLoop() coroutine to actually send the
  // message
  co_await m_sendLoop.async_send(boost::system::error_code{}, message,
                                 boost::asio::use_awaitable);

  // 2nd, if we're expecting a reply, send a channel to the ReadLoop() coroutine
  // to send the reply back to us
  boost::asio::experimental::channel<void(boost::system::error_code, DBusReply)>
      replyChannel{m_ioContext, 1};

  if (std::ranges::contains(message.GetFlags(),
                            DBusMessageFlags::NO_REPLY_EXPECTED)) {
    co_return std::nullopt; // [TODO]: We should only return here once we
                            // actually sent the message
  } else {
    co_await m_replyChannel.async_send(
        boost::system::error_code{}, &replyChannel, boost::asio::use_awaitable);
  }
