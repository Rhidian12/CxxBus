...
namespace
{
  ...
  std::vector<byte> CreateDBusMessage(uint32_t serial, std::vector<byte> messageBody, std::vector<DBusMessageFlags> const& messageFlags,
                                      std::string const& method, std::string const& objectPath, std::string const& interface, std::string const& signature,
                                      std::string const& error)
  {
    ...
    dbusMessage.append_range(
        MarshalDBusType(std::ranges::fold_left(messageFlagsCasted, static_cast<uint8_t>(0), [](uint8_t a, uint8_t b) -> uint8_t { return a | b; })));

    dbusMessage.append_range(MarshalDBusType(1));  // Major Version

    dbusMessage.append_range(MarshalDBusType(static_cast<uint32_t>(messageBody.size())));  // Length of the message body in bytes
    ...
    for (HeaderField const& headerField : HEADER_FIELDS)
    {
      ...
      dbusMessage.append_range(MarshalDBusType(std::make_tuple(static_cast<uint8_t>(headerField.decimalCode), *variant)));
    }
...
DBusMessage::DBusMessage(std::string method, std::string path, std::string interface)
  : DBusMessage(std::move(method), std::move(path), std::move(interface), "", {})
{
}

DBusMessage::DBusMessage(std::string method, std::string path, std::string interface, std::string signature, std::vector<byte> messageBody)
