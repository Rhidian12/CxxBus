...
template <typename T>
concept IsDBusType = IsDBusBasicType<T> || IsDBusContainer<T>;
...
class Variant
{
 ...
 public:
  ...
  explicit Variant(T&& value)
    : m_variantData{.signature = GetTypeSignature<T>(),
                    .dataAlignment = GetAlignmentOfDBusType<T>(),
                    .data = std::unique_ptr<void, CustomDeleter>(new T{std::forward<T>(value)},
                                                                 CustomDeleter{.deleter = [](void* data) { delete static_cast<T*>(data); }}),
                    .marshalDataFunc = [](void* data, std::vector<byte>& dbusType) { MarshalDBusTypeImpl(*static_cast<T*>(data), dbusType); },
                    .copyFunc =
                        [](void* otherData)
                    {
                      return std::unique_ptr<void, CustomDeleter>(new T{*static_cast<T*>(otherData)},
                                                                  CustomDeleter{.deleter = [](void* data) { delete static_cast<T*>(data); }});
                    }}
  {
...
{
  dbusType.reserve(sizeof(T));

  // Fixed Type
  if constexpr (std::is_same_v<T, bool>)
  {
    // Booleans are marshalled as uint32_t
    dbusType.reserve(sizeof(uint32_t));
...
void MarshalBasicStringlikeType(T const& value, std::vector<byte>& dbusType)
{
  ...
  {
    dbusType.reserve(sizeof(uint32_t) + str.size() + 1);  // sizeof uint32_t + length of message + terminating null
  ...
  {
    dbusType.reserve(sizeof(uint8_t) + str.size() + 1);  // sizeof uint8_t + length of signature + terminating null
...
void MarshalDBusArray(T const& value, std::vector<byte>& dbusType)
{
  ...
  dbusType.reserve(sizeof(uint32_t) + size);
...
void MarshalDBusStruct(T const& value, std::vector<byte>& dbusType)
{
  ...
  uint32_t size{};
  GetSizeOfDBusType<ElemType>(std::get<I>(value), size);
  dbusType.reserve(size);

  // [TODO]: Make only 1 reserve call
  if constexpr (I < MaxI - 1)
  {
    dbusType.reserve(dbusType.capacity() + GetAlignmentOfDBusType<ElemType>());
  }
...
template <IsDBusType T>
std::vector<byte> CreateDBusBody(T const& value)
{
  std::vector<byte> dbusBody{};

  if constexpr (IsDBusBasicFixedType<T>)
  {
    dbusBody = MarshalBasicFixedType(value);
  }
  else
  {
    dbusBody = MarshalBasicStringlikeType(value);
  }
...
concept IsAcceptedMessageType = requires {
  requires T == DBusMessageType::METHOD_CALL || T == DBusMessageType::METHOD_RETURN || T == DBusMessageType::ERROR || T == DBusMessageType::SIGNAL;
};

template <DBusMessageType MsgType, IsDBusType T>
std::vector<byte> CreateDBusMessage(T const& value, std::vector<DBusMessageFlags> const& messageFlags)
{
  ...
  // 2nd uint32_t is the serial of this message, used as a cookie by the sender to identify the reply correspending to this request. Must be non-zero value
  // Array of struct of byte, varaint are the header fields. The message type specifies which fields are required
  ...
  dbusMessage.append_range(MarshalDBusType(static_cast<uint8_t>(Endianness::LITTLE_ENDIAN_TYPE)));
  dbusMessage.append_range(MarshalDBusType(static_cast<uint8_t>(MsgType)));
  // Same as the old std::accumulate
  dbusMessage.append_range(MarshalDBusType(std::ranges::fold_left(messageFlags, static_cast<uint8_t>(0), [](DBusMessageFlags a, DBusMessageFlags b) -> uint8_t
                                                                  { return static_cast<uint8_t>(a) | static_cast<uint8_t>(b); })));
  dbusMessage.append_range(MarshalDBusType(1));  // Major Version

  uint32_t size{};
  GetSizeOfDBusType(value, size);
  dbusMessage.append_range(MarshalDBusType(size));  // Length of the message body in bytes

  // [TODO]: Implement proper serial
  dbusMessage.append_range(MarshalDBusType(1));  // serial

  // [TODO]: Add header fields once structs can be serialized
  for (HeaderField const& headerField : HEADER_FIELDS)
  {
    ...
    dbusMessage.append_range(MarshalDBusType(std::make_tuple(static_cast<uint8_t>(headerField.decimalCode), Variant{0})));
  }
}
